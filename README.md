...
graph TD
    ...
    subgraph Receiver (16/24-bit Parallel Conversion)
        RX -->|24-bit Left Data| RX_L[Left Channel]
        RX -->|24-bit Right Data| RX_R[Right Channel]
    end

    %% Encryption Pipeline
    subgraph Encryption Stage (Dual-Chained Stream Ciphers)
        RX_L -->|rx_left| ENC1_L[stream_cipher1 <br> LFSR Seed: 0xACE1]
        RX_R -->|rx_right| ENC1_R[stream_cipher1 <br> LFSR Seed: 0xACE1]
        
        ENC1_L -->|enc1_left| ENC2_L[stream_cipher2 <br> LFSR Seed: 0xBEEF]
        ENC1_R -->|enc1_right| ENC2_R[stream_cipher2 <br> LFSR Seed: 0xBEEF]
    end

    %% Decryption Pipeline (Symmetric XOR)
    subgraph Decryption Stage (Dual-Chained Stream Deciphers)
        ENC2_L -->|enc2_left| DEC2_L[stream_cipher2 <br> LFSR Seed: 0xBEEF]
        ENC2_R -->|enc2_right| DEC2_R[stream_cipher2 <br> LFSR Seed: 0xBEEF]
        
        DEC2_L -->|dec2_left| DEC1_L[stream_cipher1 <br> LFSR Seed: 0xACE1]
        DEC2_R -->|dec2_right| DEC1_R[stream_cipher1 <br> LFSR Seed: 0xACE1]
    end

    %% Transmitter Serialization
    subgraph Transmitter (24/16-bit Parallel Conversion)
    ...
    %% Formatting
    style Encryption Stage fill:#3f1d5e,stroke:#9f5afd,stroke-width:2px;
    style Decryption Stage fill:#1b4d3e,stroke:#39c5bb,stroke-width:2px;
    style Receiver fill:#2b2b2b,stroke:#888,stroke-width:1px;
    style Transmitter fill:#2b2b2b,stroke:#888,stroke-width:1px;
```
